...
**`maxima-cli ui-backend` — the persistent server mode (added 2026-07-04, reworked same day).** One long-running process holds the logged-in session, LSX server, `/authorize` endpoint and RTM presence — the role the egui UI's bridge_thread plays in-process — and speaks JSONL over stdio: requests in (`{"id":N,"cmd":"list-games"|"friends"|"launch"|"install",…}`), matched responses and pushed events out (`ready`, `presence` diffs, `install-progress`/`install-done`, `game-started`/`game-stopped`). Exits on stdin EOF. **Architecture-aligned with upstream PR [#23](https://github.com/ArmchairDevelopers/Maxima/pull/23) ("Maxima Server": frontends as thin clients of one server process)** — that branch is a stale draft whose server→client notification layer is unfinished; this is the pragmatic fork-side form, notifications included. Protocol documented in [maxima-cli/src/ui_backend.rs](maxima-cli/src/ui_backend.rs).

**CleanSpawn — why GUI-spawned games froze.** TF2 launched from a `.app` via `Foundation.Process` froze right after LSX `GetAllGameInfo`; the identical chain from Terminal reached the menu. Draconis root-caused this exact signature (its `Services/CleanSpawn.swift`): children inherit the app's non-CLOEXEC descriptors, session, and App Nap responsibility, which wine's macOS driver chokes on. `maxima-native` spawns the ui-backend with the same technique (`posix_spawn` + `POSIX_SPAWN_CLOEXEC_DEFAULT` + `POSIX_SPAWN_SETSID` + best-effort `responsibility_spawnattrs_setdisclaim`), so the wine tree below it behaves as if Terminal-launched.
