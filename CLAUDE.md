...
- **Auto-start**: `server::ensure_server_running` (CLI) and `maxima_lib::server_client::ensure_running` (egui/TUI) probe the port and spawn `maxima-cli server` detached if it's down. So launching any frontend brings the server up when it isn't already (and wasn't started at logon). The egui/TUI keep their own in-process session and their `start_lsx` defers to the server's LSX when it's bound first — full thin-client conversion of those two is the deferred larger PR #23 refactor (gut `maxima-lib` into a proto client); the **SwiftUI app is a true thin client** (connects over TCP, no in-process session).
