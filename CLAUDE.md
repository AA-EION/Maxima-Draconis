...
- **CLI control**: `maxima-cli server-stop` and `server-status [--json]` are pure client commands (no login — short-circuited before session setup). `launch`/`install` **forward to a running server FIRST** — the forward check sits at the very top of startup, before any login / wine setup, so a forwarded command does no redundant in-process work (verified: `maxima-cli launch` → "Forwarding launch to the Maxima server" → game runs, `playing: true`). Only when **no** server is running does it fall back to the standalone in-process path (Draconis's contract; the CEG-fix install flags always stay in-process). Spawn target is the `maxima-server` binary.
