...
maxima-cli/          CLI frontend — `maxima-cli launch <slug>` (legacy
                     ...
                     auth-only mode), `maxima-cli server` (the multi-client
                     server — see "Maxima server" below), `server-stop` /
                     `server-status`, plus utility subcommands. `server.rs`
                     is the server + client helpers; `tray.rs` is the
                     Windows tray (winapi, windows-only).
...
**Thin-client status (which frontends hold NO in-process `Maxima`):**
- **SwiftUI Maxima.app** — true thin client (TCP, no session). ✅
- **TUI (`maxima-tui`)** — **fully converted** ([service.rs](maxima-tui/src/service.rs) holds a `MaximaClient`, renders the server's real library; verified: launching it raises the server's client count). Keeps `maxima-lib` only for host utilities (logging, registry check). ✅
...
- **CLI is now a pure client (dead code removed).** Deleted the unreachable in-process product commands from [maxima-cli/src/main.rs](maxima-cli/src/main.rs) (`install_game`, `verify_game`, `bottle_info`, `games_json` + structs, `download_specific_file`, `do_cloud_sync`, `canonical_slug`, and the `Launch { login: None }` resolution arm) — all forwarded to `maxima-server` by the forward-first block. `locate-game` now forwards too (`server::run_locate_game`). main.rs 2575 → 1391 lines. Kept in-process on purpose: manual/offline `launch --login`, `serve`, the interactive menu, the developer/diagnostic subcommands. Builds clean darwin + windows-gnu.
- **Server owns the status-bar icon on every OS** — new [maxima-server/src/status_icon.rs](maxima-server/src/status_icon.rs) dispatcher: Windows tray (now the embedded `logo.ico`), macOS spawns `Maxima.app --menubar` (a new `.accessory` `MenuBarExtra` mode; `--menubar` flag), Linux SNI tray via `ksni` behind the off-by-default `linux-tray` feature (CI checks it — can't cross-check on macOS). See the "Bar icon" bullet above.
- **Stable install path `~/Library/Application Support/Maxima/bin/`** — `maxima_lib::server_client::app_support_bin_dir()` + `ensure_app_support_install()` (server self-syncs its sibling binaries + `MaximaBootstrap.app` there on startup, macOS best-effort). Rust + Swift server-resolvers check it; the launchd plist points at it and now runs the `maxima-server` binary (the old `maxima-cli server` subcommand is gone). Rationale: app-translocation makes `.app`-internal paths unstable — see [docs/MACOS_BUNDLING.md](docs/MACOS_BUNDLING.md).
- **Native SwiftUI: friends rail on the right + app icon.** Friends moved from a left-nav page to a persistent right-hand rail (online-first, in-game status). `build.sh` generates `AppIcon.icns` from the logo and Info.plist references it, so the launcher carries the brand mark.
- **Server independence + install/uninstall + boot policy.** The server is no longer owned by any frontend: the SwiftUI app (and the Rust `ensure_running` paths) spawn it into its **own session** (`POSIX_SPAWN_SETSID` / `setsid()` via `pre_exec`) so closing the app doesn't reap it (it was inside the app's launchd job before — the "server dies when I close the window" bug). New [maxima-lib/src/service.rs](maxima-lib/src/service.rs) + `maxima-cli service install|uninstall|status` register/unregister the server with the OS and set a **boot policy** (persisted to `config.json`, read by every frontend): `auto` (launchd LaunchAgent / HKCU Run / systemd user unit), `on-demand` (default — a frontend/game spawns the detached server), `manual` (explicit only). macOS uses a **classic LaunchAgent, not `SMAppService`** (which needs a real signing identity — ad-hoc doesn't work; grounded by research). `service uninstall [--purge]` leaves **no trace** (agent, LaunchServices protocol claims, binaries, config). SwiftUI Settings has the boot-policy picker + uninstall; [maxima-native/make-pkg.sh](maxima-native/make-pkg.sh) builds a PKG installer + uninstaller. See [docs/MACOS_BUNDLING.md](docs/MACOS_BUNDLING.md) §0–0c, 3b.
- **Still open** (task, not a regression): the egui `maxima-ui` bridge rewire onto `MaximaClient` — it's the last frontend holding an in-process `Maxima`. Server side has been ready since 2026-07-05; the remaining work is inside `maxima-ui` (see [docs/CLEANUP.md](docs/CLEANUP.md) §6). The egui Settings boot-policy picker (parity with SwiftUI) is a small follow-up once that lands. Native-app box art is the other known parity gap (list-games JSON carries no image URLs yet; the server's `game-images` RPC exists for when it's wired).

### 2026-07-05 — Multi-client Maxima server (PR #23 architecture) + per-OS bar icon + autostart

The single-client stdio `ui-backend` became a real **multi-client TCP server** (`maxima-cli server`, port 13220) — upstream PR #23's "one server, frontends as thin clients, state synced" design, notifications included. See the "Maxima server" section up top. What landed:

- **`maxima-cli/src/server.rs`** — TCP loopback server, many concurrent clients, one shared session (LSX + `/authorize` + RTM). Per-client request dispatch; events broadcast to ALL clients (`ready`, `presence`, `install-*`, `game-started`/`game-stopped`). Client helpers: `is_running`, `send_shutdown`, `print_status`, `ensure_server_running`, `forward_streaming`.
- **CLI control** — `server-stop` / `server-status [--json]` (pure client, no login); `launch`/`install` forward to a running server (state syncs) with the standalone in-process path preserved when none runs.
- **Auto-start on frontend launch** — `maxima_lib::server_client::ensure_running` (egui + TUI) and `server::ensure_server_running` (CLI) spawn `maxima-cli server` detached if the port is down. SwiftUI Maxima.app is now a **true thin client** (Backend.swift over TCP, no in-process session; its CleanSpawn removed — the game disclaim now lives at the cxstart hop in Rust).
- **Bar icon per OS** — Windows native `Shell_NotifyIcon` tray in the server (`tray.rs`, winapi, no new deps: Open Maxima / Stop Server); macOS `MenuBarExtra` in Maxima.app; Linux headless. All expose Open-UI + Stop-Server; the tray/menu are decoupled clients of the port.
- **Logon autostart** — NSIS `HKCU\…\Run\MaximaServer` (uninstaller cleans up); `installer/autostart/` launchd plist (macOS) + systemd user unit (Linux) with install instructions.
- **`run_via_cxstart` exit detection** — switched to `/usr/bin/pgrep -if` (sysinfo can't read wine/Rosetta cmdlines; absolute path survives launchd's minimal PATH); correctness anchored by the LSX-connection guard in `update_playing_status`.

Deferred (honest scope): fully gutting `maxima-lib` into a thin proto client per PR #23's letter — the egui/TUI keep their in-process session and merely defer LSX to the server. The multi-client value (shared state, one tray/menu, CLI control, autostart) is delivered without that refactor.
...
Fifth follow-up — **`ui-backend` server mode + maxima-native parity rework**. The per-action CLI-spawn architecture couldn't carry live state (friends presence, download queue) and GUI-spawned game launches froze after `GetAllGameInfo` (FD/session/App-Nap inheritance — Draconis's CleanSpawn root cause, same signature). Fix: new `maxima-cli ui-backend` persistent JSONL server (aligned with upstream PR #23's "Maxima Server" direction, including the server→client notifications that draft lacks), spawned from the Swift app via a CleanSpawn-style `posix_spawn`. maxima-native gains feature parity with the egui UI: Friends view with live presence (sorted online-first, in-game labels), Downloads view with real-time progress + sidebar badges, per-game settings sheet (launch args / exe override / cloud saves), backend connection footer with reconnect. `games_json` extracted for reuse; `launch` requests honor per-game args/exe/cloud-saves.
