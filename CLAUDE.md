...
**`maxima-cli server` — the multi-client Maxima server (added 2026-07-04; superseded the single-client `ui-backend`).** One long-running process holds the logged-in session, LSX server, `/authorize` endpoint and RTM presence, and serves **many concurrent clients** over loopback TCP (default `127.0.0.1:13220`, `MAXIMA_SERVER_PORT` to override). This is upstream PR [#23](https://github.com/ArmchairDevelopers/Maxima/pull/23)'s "Maxima Server" architecture — one server, frontends as thin clients, state synced — in its pragmatic fork form, including the server→client notification layer that draft leaves unfinished. Implemented in [maxima-cli/src/server.rs](maxima-cli/src/server.rs).
...
- **CLI control**: `maxima-cli server-stop` and `server-status [--json]` are pure client commands (no login — short-circuited before session setup). `launch`/`install` **forward to a running server** when one is up (so state syncs across clients) and fall back to the standalone in-process path when none is (Draconis's contract unchanged; the CEG-fix install flags always stay in-process).
...
- **egui UI (`maxima-ui`)** — being converted: the proto DTOs carry the image URLs (avatar / game art / hero) and download byte counts it needs, and its `bridge_thread`/`event_thread` are rewired onto `MaximaClient`, **reusing** the existing image-loader thread (now fed URLs from the server instead of `ServiceLayer`).
- **Bar icon, per-OS idiom** (all three do Open-UI / Stop-Server): **Windows** — native `Shell_NotifyIcon` tray *inside the server* ([tray.rs](maxima-cli/src/tray.rs), winapi, no new deps), decoupled as an ordinary client of the port. **macOS** — SwiftUI `MenuBarExtra` in Maxima.app (status items need a GUI app; the launchd-run server is otherwise headless). **Linux** — headless by design (no GTK pulled into the CLI); drive it with `maxima-cli` + the systemd unit.
